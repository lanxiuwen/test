...
CumulativeSums(int n)
{
	...
	z = (sup > -inf) ? sup : -inf;
	//The final S is the same both in forward and backward test.
	//When the sup is maximum in forward test, (S-sup) is minimum in backward test. So is the inf.
	//miraculous! (S-inf) > -(S-sup) is easier to understand.	
